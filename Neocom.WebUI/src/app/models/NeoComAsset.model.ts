...
export class NeoComAsset extends NeoComNode {
  ...
  */
      ...
      if (this.downloaded) {
        ...
        appModelStore.getBackendContainerContents(this.assetID)
          .subscribe(result => {
            ...
            this.contents = this.processDownloadedAssetsInternal(result);
  ...
  private processDownloadedAssetsInternal(assets: NeoComNode[]): NeoComAsset[] {
    ...
    for (let node of assets) {
      switch (node.jsonClass) {
        case "NeoComAsset":
          ...
          break;
        case "SpaceContainer":
          let container = new NeoComNode(node);
