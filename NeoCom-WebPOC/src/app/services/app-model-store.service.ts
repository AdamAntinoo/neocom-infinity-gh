...
export class AppModelStoreService {
  ...
  public accessAssets4Location(loginid: string, characterid: number, locationid: number): Observable<NeoComNode[]> {
    ...
    return this.http.get(AppModelStoreService.RESOURCE_SERVICE_URL + "/login/" + loginid + "/pilot/" +characterid + "/assetsmanager/location/" + locationid + "/downloadcontents")
      .map(res => res.json())
      .map(result => {
        return result;
      });
