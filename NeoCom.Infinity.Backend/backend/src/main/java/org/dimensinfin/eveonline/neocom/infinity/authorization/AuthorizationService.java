...
//		this.esiDataAdapter = esiDataAdapter;
	...
	public Credential validateAuthorizationToken(final String authCode, final String state, final String dataSource) throws SQLException, NeoComException, IOException {
		logger.info(">> [AuthorizationService.validateAuthorizationToken]");
		final Instant timer = Instant.now();
		this.validateStateMatch(state);
	...
	private TokenTranslationResponse getTokenTranslationResponse(final TokenVerification store) throws NeoComException, IOException {
		...
		if (response.isSuccessful()) {
			...
			throw new NeoComException("NEO [AuthorizationService.getTokenTranslationResponse]> the TokenTranslationResponse response is" +
					" invalid. "
					+ response.message());
	}
	...
	private VerifyCharacterResponse getVerifyCharacterResponse(final TokenVerification store) throws NeoComException, IOException {
		...
		if (verificationResponse.isSuccessful()) {
			...
			throw new NeoComException("NEO [AuthorizationService.getVerifyCharacterResponse]> the VerifyCharacterResponse response is " +
					"invalid. " +
