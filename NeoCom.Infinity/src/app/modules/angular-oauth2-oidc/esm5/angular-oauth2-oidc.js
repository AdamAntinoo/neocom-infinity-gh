import { __awaiter, __generator, __extends, __values, __spread, __read } from 'tslib';
...
var JwksValidationHandler = /** @class */ (function (_super) {
    ...
    function JwksValidationHandler() {
        ...
        _this.cyptoObj = window.crypto || ((window)).msCrypto;
        _this.textEncoder = new ((window)).TextEncoder();
        return _this;
    }
    JwksValidationHandler.prototype.validateSignature = function (params, retry) {
        ...
        return __awaiter(this, void 0, void 0, function () {
            var _this = this;
            var kid, keys, key, alg, kty_1, matchingKeys, error, error, error, _a, header, body, sig, cyptokey, isValid;
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        ...
                        kid = params.idTokenHeader['kid'];
                        keys = params.jwks['keys'];
                        alg = params.idTokenHeader['alg'];
                        if (kid) {
                            key = keys.find(function (k) { return k['kid'] === kid; });
                        }
                        else {
                            kty_1 = this.alg2kty(alg);
                            matchingKeys = keys.filter(function (k) { return k['kty'] === kty_1 && k['use'] === 'sig'; });
                            if (matchingKeys.length > 1) {
                                error = 'More than one matching key found. Please specify a kid in the id_token header.';
                                console.error(error);
                        ...
                        if (!key && retry && kid) {
                            error = 'expected key not found in property jwks. ' +
                        ...
                        _a = __read(params.idToken.split(','), 3), header = _a[0], body = _a[1], sig = _a[2];
                        return [4 /*yield*/, this.cyptoObj.subtle.importKey('jwk', (key), alg, true, ['verify'])];
                    case 1:
                        cyptokey = _b.sent();
                        return [4 /*yield*/, this.cyptoObj.subtle.verify(alg, cyptokey, this.textEncoder.encode(sig), this.textEncoder.encode(body))];
                    case 2:
                        isValid = _b.sent();
                        if (isValid) {
                            ...
                            return [2 /*return*/, Promise.reject('Signature not valid')];
                        }
                        return [2 /*return*/];
                }
            });
    ...
    JwksValidationHandler.prototype.calcHash = function (valueToHash, algorithm) {
        return __awaiter(this, void 0, void 0, function () {
            var valueAsBytes, resultBytes;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        valueAsBytes = this.textEncoder.encode(valueToHash);
                        return [4 /*yield*/, this.cyptoObj.subtle.digest(algorithm, valueAsBytes)];
                    case 1:
                        resultBytes = _a.sent();
                        return [2 /*return*/, String.fromCharCode.apply(null, new Uint16Array(resultBytes))];
                }
            });
        });
    };
